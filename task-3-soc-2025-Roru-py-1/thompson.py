...
class ThompsonSamplingAgent(Agent):
    ...
    def __init__(self, time_horizon, bandit:MultiArmedBandit,): 
        ...
        self.reward_memory = np.zeros(len(self.bandit.arms))
        self.count_memory = np.zeros(len(self.bandit.arms))
        self.time_step =0

    def give_pull(self):
        tpsn = np.zeros(len(self.bandit.arms))
        for i in range(len(self.bandit.arms)):
            tpsn[i] = np.random.beta(self.reward_memory[i] + 1, self.count_memory[i] - self.reward_memory[i] + 1)
        best_arm = np.argmax(tpsn)
    ...
    def plot_cumulative_regret(self):
        ...
        plt.plot(timesteps, self.bandit.cumulative_regret_array, linestyle='-', color='y', label='Thompson')
        ...
        plt.plot(timesteps, avg_rewards, linestyle='-', color='y', label='Thompson')
