...
class UCBAgent(Agent):
    ...
    def give_pull(self):
        if self.time_step < len(self.bandit.arms):
            ...
            exp_bonus = np.sqrt(2*math.log(self.time_step+1)/self.count_memory)
            ucb = emp_mean + exp_bonus
    ...
    def plot_cumulative_regret(self):
        ...
        plt.plot(timesteps, self.bandit.cumulative_regret_array, linestyle='-', color='g', label='UCB')
        ...
        plt.plot(timesteps, avg_rewards, linestyle='-', color='g', label='UCB')
...
if __name__ == "__main__":
    ...
    agent = UCBAgent(TIME_HORIZON,bandit)
    # Loop
